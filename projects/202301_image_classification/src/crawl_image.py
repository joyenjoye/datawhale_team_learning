...
def crawl_single_class(keyword, DOWNLOAD_NUM=200):
    if os.path.exists('data/'+keyword):
        print('文件夹 da ta/{} 已存在，之后直接将爬取到的图片保存至该文件夹中'.format(keyword))
    else:
        os.makedirs('data/{}'.format(keyword))
        print('新建文件夹:data/{}'.format(keyword))
    count = 1
                        ...
                        for x in json_data:
                            ...
                            if type not in ["gif"]:
                                ...
                                try:
                                    ...
                                    # 保存文件名
                                    # file_save_path = f'data/{keyword}/{num}-{fromPageTitleEnc}.{type}'
                                    file_save_path = f'data/{keyword}/{num}.{type}'
...
if __name__ == "__main__":
    ...
    for each in class_list:
        crawl_single_class(each, DOWNLOAD_NUM=200)
